...
@userge.on_cmd("help", about={'header': "Guide to use UsergeBlack commands"}, allow_channels=False)
...
if userge.has_bot:
    def check_owner(func):
        async def wrapper(_, c_q: CallbackQuery):
            if c_q.from_user and c_q.from_user.id in Config.OWNER_ID:
                ...
                await c_q.answer(
                    f"Only {user_dict['flname']} can do that! Nice try...",
                    show_alert=True)
        return wrapper
    ...
    async def callback_back(callback_query: CallbackQuery):
        ...
        pos_list = cur_pos.split('|')
        if len(pos_list) == 1:
            await callback_query.answer("You are in the Main-Menu.", show_alert=True)
            return
        if len(pos_list) == 2:
            text = "🖥 **UsergeBlack Main Menu** 🖥"
    ...
    async def callback_mm(callback_query: CallbackQuery):
        await callback_query.edit_message_text(
            "🖥 **UsergeBlack Main Menu** 🖥", reply_markup=InlineKeyboardMarkup(main_menu_buttons()))
    ...
    async def prvt_msg(_, c_q: CallbackQuery):
        ...
        if msg_id not in PRVT_MSGS:
            await c_q.answer("message outdated!", show_alert=True)
            return
        ...
        if c_q.from_user.id == user_id or c_q.from_user.id in Config.OWNER_ID:
            ...
            await c_q.answer(
                f"Sorry, only {flname} can view that private message 😔", show_alert=True)
        ...
        results = [
            InlineQueryResultArticle(
                ...
                input_message_content=InputTextMessageContent(
                    "**Here's how to setup UsergeBlack** 😎"
                ),
                url="https://github.com/BlackStoneOfficial/UsergeBlack",
                description="Get your own",
                thumb_url="https://imgur.com/download/AbBKEaX",
        ...
        if inline_query.from_user and inline_query.from_user.id in Config.OWNER_ID:
            results.append(
                InlineQueryResultArticle(
                    ...
                    title="Main Menu",
                    input_message_content=InputTextMessageContent(
                        "🖥 **UsergeBlack Main Menu** 🖥"
                    ),
                    url="https://github.com/BlackStoneOfficial/UsergeBlack",
                    description="UsergeBlack Main Menu",
                    thumb_url="https://imgur.com/download/AbBKEaX",
                    reply_markup=InlineKeyboardMarkup(main_menu_buttons())
                )
            )
            if '-' in inline_query.query:
                ...
                if pm_inline_msg:
                    ...
                    text = f"Hello, welcome to **{owner.first_name}**'s Inbox.\n\nWhat you want to do ?"
                buttons = [[
                    InlineKeyboardButton(
                ...
                results.append(
                    InlineQueryResultArticle(
                        ...
                        description="Inline PMPermit-Handler",
                        thumb_url="https://imgur.com/download/AbBKEaX",
                        reply_markup=InlineKeyboardMarkup(buttons)
                    )
                )
