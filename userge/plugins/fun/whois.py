...
async def who_is(message: Message):
    await message.edit("`Let's see who this user really is...`")
    ...
    if from_user or from_chat is not None:
        ...
        message_out_str += f"<b>🏢 Datacenter:</b> <code>{from_user.dc_id}</code>\n"
        message_out_str += f"<b>🤖 Is Bot:</b> <code>{from_user.is_bot}</code>\n"
        message_out_str += f"<b>🚫 Is Restricted:</b> <code>{from_user.is_restricted}</code>\n"
