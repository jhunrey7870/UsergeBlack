...
@userge.on_cmd("gban", about={
    ...
    'description': "Adds a User to the GBAN-List "
...
async def gban_user(message: Message):
    """ ban a user globally """
    await message.edit("`GBAN-ning...`")
    user_id, reason = message.extract_user_and_text
    if not user_id:
        await message.edit(
            "`no valid user_id or message specified.`", del_in=5)
        return
    ...
    if not reason:
        await message.edit(
            ...
            "Aborted because no reason was given.", del_in=5)
        return
    user_id = get_mem['id']
    if user_id == (await message.client.get_me()).id:
        await message.edit(r"Why would I GBAN myself?")
        return
    if user_id in Config.SUDO_USERS:
        await message.edit(
            "That user is a SUDO, I can't GBAN it.", del_in=5)
        return
    found = await GBAN_USER_BASE.find_one({'user_id': user_id})
    if found:
        await message.edit(
            "**#Already_GBanned**\n\nThat user was already GBAN-ned.\n"
            f"**Reason:** `{found['reason']}`", del_in=5)
        return
...
async def ungban_user(message: Message):
    ...
    await message.edit("`un-gbanning...`")
    ...
    found = await GBAN_USER_BASE.find_one({'user_id': user_id})
    if not found:
        await message.err("That user was never GBAN-ned.")
        return
...
@userge.on_cmd("glist", about={
    ...
    'description': "Get Up-to-date list of users GBAN-ned by you.",
    'examples': "{tr}glist"},
...
async def whitelist(message: Message):
    ...
    if found:
        await message.err("This user is aready whitelisted!")
        return
...
async def rmwhitelist(message: Message):
    ...
    found = await WHITELIST.find_one({'user_id': user_id})
    if not found:
        await message.err("That user was never whitelisted!")
        return
    await asyncio.gather(
        ...
        if gbanned:
            ...
            await asyncio.gather(
                ...
                message.reply(
                    ...
                    "\n\nI smell a GBAN-ned user here.\n\n"
                    f"**User:** [{first_name}](tg://user?id={user_id})\n"
                    f"**ID:** `{user_id}`\n**Reason:** `{gbanned['reason']}`\n\n"
                    "**Action:** Banned", del_in=10),
                CHANNEL.log(
                    r"\\**#Antispam_Log**//"
                ...
                if intruder:
                    await asyncio.gather(
                        ...
                        CHANNEL.log(
                            ...
                            f"\n**User:** [{first_name}](tg://user?id={user_id})\n"
                            f"**ID:** `{user_id}`\n**Reason:** `{intruder.reason}`\n"
                            f"**Action:** Banned in {message.chat.title}\n\n"
