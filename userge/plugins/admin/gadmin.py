...
async def promote_usr(message: Message):
    ...
    await message.edit("`Promoting...`")
    user_id, custom_rank = message.extract_user_and_text
    if not user_id:
        await message.edit(
            text="`no valid user_id or message specified.`", del_in=5)
        return
    if custom_rank:
    ...
    try:
        ...
        await message.edit("`Promoted! Now dance.`", del_in=5)
        await CHANNEL.log(
            "#PROMOTE\n\n"
        ...
        await message.edit("`invalid username!`", del_in=5)
    except PeerIdInvalid:
        await message.edit(
            "`invalid id!`", del_in=5)
    except UserIdInvalid:
        await message.edit("`invalid id!`", del_in=5)
...
async def demote_usr(message: Message):
    ...
    try:
        ...
        await message.edit("`Demoted! The dance's over.`", del_in=5)
        await CHANNEL.log(
            "#DEMOTE\n\n"
            f"USER: [{get_mem.user.first_name}](tg://user?id={get_mem.user.id}) "
            f"(`{get_mem.user.id}`)\n"
...
async def unban_usr(message: Message):
    ...
    try:
        ...
        await message.edit("`Unbanned!`", del_in=5)
        await CHANNEL.log(
            "#UNBAN\n\n"
            f"USER: [{get_mem.user.first_name}](tg://user?id={get_mem.user.id}) "
            f"(`{get_mem.user.id}`)\n"
...
async def unmute_usr(message: Message):
    ...
    try:
        get_mem = await message.client.get_chat_member(chat_id, user_id)
        await message.client.unban_chat_member(chat_id, user_id)
        await message.edit("`Unmuted!`", del_in=5)
        await CHANNEL.log(
            "#UNMUTE\n\n"
            f"USER: [{get_mem.user.first_name}](tg://user?id={get_mem.user.id}) "
            f"(`{get_mem.user.id}`)\n"
...
async def zombie_clean(message: Message):
    ...
    if rm_delaccs:
        ...
        del_admins = 0
        del_total = 0
        del_stats = r"`No deleted accounts found in this group.`"
        if can_clean:
            await message.edit("`Removing deleted accounts from this chat...`")
            ...
            if del_admins > 0:
                ...
                \n🛡 **{del_admins}** `deleted admin accounts were skipped!`"
            else:
                del_stats = f"`👻 Found` **{del_total}** `total zombies..`\
                \n`🗑 Cleaned` **{del_users}** `zombie (deleted) accounts from this chat..`"
            ...
            await message.edit(r"`I can't clean deleted accounts from here, sorry.`", del_in=5)
    else:
        del_users = 0
        del_stats = r"`No deleted accounts found in this group.`"
...
async def chatpic_func(message: Message):
    ...
    if gpic_set:
        if message.reply_to_message.photo:
            ...
            await message.edit(
                text="`no valid message/picture reply specified,`", del_in=5)
    elif gpic_del:
        try:
        ...
        await message.edit("`invalid flag type.`", del_in=5)
...
async def smode_switch(message: Message):
    ...
    if seconds:
        try:
            ...
            await message.edit(
                f"`Set the slow mode to {seconds} seconds!`", del_in=5)
            await CHANNEL.log(
                f"#SLOW_MODE\n\n"
                f"CHAT: `{message.chat.title}` (`{chat_id}`)\n"
        ...
        try:
            ...
            await message.edit(f"`Set the slow mode to {minutes} minutes!`", del_in=5)
            await CHANNEL.log(
                f"#SLOW_MODE\n\n"
                f"CHAT: `{message.chat.title}` (`{chat_id}`)\n"
        ...
        try:
            ...
            await message.client.set_slow_mode(chat_id, smode_time)
            await message.edit("`Set the slow mode to 1 hour!`", del_in=5)
            await CHANNEL.log(
                f"#SLOW_MODE\n\n"
                f"CHAT: `{message.chat.title}` (`{chat_id}`)\n"
        ...
        try:
            ...
            await message.edit("`Turned off slow mode!`", del_in=5)
            await CHANNEL.log(
                f"#SLOW_MODE\n\nCHAT: `{message.chat.title}` (`{chat_id}`)\nSLOW MODE: `Off`")
        except Exception as e_f:
            await message.edit(
        ...
        await message.edit("`inavlid flag type/mode.`", del_in=5)
