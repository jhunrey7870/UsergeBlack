...
async def lock_perm(message: Message):
    ...
    if not lock_type:
        await message.err(r"I can't lock nothing!")
        return
    if lock_type == "all":
        try:
            await message.client.set_chat_permissions(chat_id, ChatPermissions())
            await message.edit("**🔒 Locked all permission in this Chat!**", del_in=5)
            await CHANNEL.log(
                f"#LOCK\n\nCHAT: `{message.chat.title}` (`{chat_id}`)\n"
                f"PERMISSIONS: `All`")
        except Exception as e_f:
            await message.edit(
                r"`I don't have permissions to do that`\n\n"
    ...
    if lock_type in _types:
        ...
        await message.err(r"Invalid Lock-type!")
        return
    try:
...
async def unlock_perm(message: Message):
    ...
    chat_id = message.chat.id
    if not unlock_type:
        await message.err(r"I can't unlock nothing!")
        return
    if unlock_type == "all":
        try:
            await message.client.set_chat_permissions(
            ...
            await message.edit(
                "**🔓 Unlocked all permissions in this Chat!**", del_in=5)
            await CHANNEL.log(
                f"#UNLOCK\n\nCHAT: `{message.chat.title}` (`{chat_id}`)\n"
                f"PERMISSIONS: `All`")
        except Exception as e_f:
...
async def view_perm(message: Message):
    ...
    await message.edit("`Checking group permissions...`")
    ...
    permission_view_str = ""
    permission_view_str += "<u>Permissions of this Chat:</u>\n\n"
