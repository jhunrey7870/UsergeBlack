...
@userge.on_cmd("update", about={
    'header': "Check Updates or Update UsergeBlack",
    'flags': {
        '-pull': "pull updates",
...
async def check_update(message: Message):
    ...
    if not (pull_from_repo or push_to_heroku):
        if out:
            ...
            await message.edit(f'**UsergeBlack is up-to-date with [{branch}]**', del_in=5)
        return
    if pull_from_repo:
        if out:
            ...
            if not push_to_heroku:
                await message.edit('**UsergeBlack has successfully Updated!**\n'
