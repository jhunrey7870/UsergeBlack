...
class UsergeBot(_AbstractUserge):
    ...
    def __init__(self, **kwargs) -> None:
        _LOG.info(_LOG_STR, "Setting UsergeBlackBot Configs")
...
class Userge(_AbstractUserge):
    ...
    def __init__(self, **kwargs) -> None:
        _LOG.info(_LOG_STR, "Setting UsergeBlack configs")
        kwargs = {
    ...
    async def start(self) -> None:
        ...
        _LOG.info(_LOG_STR, "Starting UsergeBlack")
        await super().start()
        if self._bot is not None:
            _LOG.info(_LOG_STR, "Starting UsergeBlackBot")
    ...
    async def stop(self) -> None:  # pylint: disable=arguments-differ
        """ stop client and bot """
        if self._bot is not None:
            _LOG.info(_LOG_STR, "Stopping UsergeBlackBot")
            await self._bot.stop()
        _LOG.info(_LOG_STR, "Stopping UsergeBlack")
        await super().stop()
        _close_db()
    ...
    def begin(self, coro: Optional[Awaitable[Any]] = None) -> None:
        ...
        async def _shutdown(sig: signal.Signals) -> None:
            _LOG.info(_LOG_STR, f"Received stop-Signal [{sig.name}], Exiting UsergeBlack...")
            await _finalize()
        ...
        logbot.edit_last_msg("UsergeBlack started up successfully!")
        logbot.end()
        ...
        try:
            if coro:
                ...
                _LOG.info(_LOG_STR, f"Idling UsergeBlack - {mode}")
                idle()
